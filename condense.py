...
def matchloss(args, img_real, img_syn, lab_real, lab_syn, model,it):
    ...
    if args.match == 'feat':
        ...
        for i in range(len(g_real)):
            if (len(g_real[i].shape) == 1) and not args.bias:  
                continue
...
def Conloss(args, img_real, img_syn, lab_real, lab_syn, model):
    loss = None
    criterion = nn.CrossEntropyLoss()
...
def condense(args, logger, device='cuda'):
    ...
    for it in range(args.niter):
        ...
        model.load_state_dict(current_state_dict)
        model_param = [device, nclass, path+file_dir_set[pth_idx]]
        ...
        for ot in range(args.inner_loop):
            ...
            for c in range(nclass):
                ...
                if it<200:
                    label_false=(torch.ones(lab.shape[0],dtype=lab.dtype)).cuda()*((lab[0]+1)%nclass)
                    label_syn_false=(torch.ones(lab_syn.shape[0],dtype=lab_syn.dtype)).cuda()*((lab_syn[0]+1)%nclass)
                    loss = loss+Conloss(args, img_aug[:n], img_aug[n:], label_false, label_syn_false, model)
